...
public class SafetyTests {
  ...
  /**
   * Check steam boiler goes into emergency stop when steam level non-zero during
   * initialisation.
   */
  @Test
   ...
   * difficult to simulate properly. This case is done by setting an evacuation
   * rate above the maximum pump capacity and forcing the valve open. This has to
   * be done after initialisation as well, since otherwise it would emergency stop
  ...
  public void safetytest_08() {
    ...
    // steam boiler! With four pumps at DEFAULT 4L/s, that's a maximum filling of
    // 16L/s.
    ...
    // At this point, we have to wait for the emergency stop event. This can take
    // some time as the
  ...
  /**
   * Check emergency stop when water falls to minimum limit level. This is
   * difficult to simulate properly. This case is done by breaking all pumps so
   * that water cannot be refilled. This has to be done after initialisation as
  ...
  public void safetytest_09() {
    ...
    // At this point, we have to wait for the emergency stop event. This will take a
    // long time as
    // the boiler will have to empty by exhausting steam only. At a maximum exhaust
    // of 10L/s it will
    // take around 50s to empty the entire boiler (assuming it was full, which it
    // could not be)/
   ...
   * pump capacity and locking the only pump into the on position. In turn, this
   * forces the system into degraded mode but it will not emergency stop until the
   * limit is reached. This has to be done after initialisation as well, since
  ...
  public void safetytest_10() {
    ...
    // wrong during this time.
    clockForWithout(240, controller, model, atleast(MODE_emergencystop));
    // Under ideal conditions, should get here without problems. Now, we break the
    // pump by forcing
    ...
    // At this point, we have to wait for the emergency stop event. This can take
    // some time as the
    // boiler will have to fill up. For a max limit of 400L at 20L/s this would take
    // around 20s.
    // Note, however, that steam exhaust affects this calculation. That said, we
    // also know we must
  ...
  public void safetytest_11() {
    ...
    // wrong during this time.
    clockForWithout(240, controller, model, atleast(MODE_emergencystop));
    // Under ideal conditions, should get here without problems. Now, break both
    // level and steam
    // sensor.
    model.setLevelSensor(new LevelSensorModels.StuckNegativeOne(model));
    model.setSteamSensor(new SteamSensorModels.StuckNegativeOne(model));
  ...
  public void safetytest_12() {
    ...
    clockForWithout(240, controller, model, atleast(MODE_emergencystop));
    // Under ideal conditions, should get here without problems. Now, break the
    // steam sensor forcing
    // the system into degraded mode. It should be able to carry on for a while
    // though like this.
    model.setSteamSensor(new SteamSensorModels.StuckNegativeOne(model));
